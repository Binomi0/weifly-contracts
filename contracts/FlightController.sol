...
contract FlightController {
  ...
  constructor(address _defaultAdmin, address _aircraftAddress, AirlineCoin _airlineCoin) {
  ...
  function _calculateRewards(address _pilot) private view returns (uint256) {
    return (flightDetails[_pilot].distance / 10) * flightDetails[_pilot].multiplier;
  }

  function startFlight(uint _flightId, uint _distance, uint _aircraftId, uint _multiplier) public nonReentrant {
    require(
      flightDetails[msg.sender].startTime == 0 || block.timestamp >= flightDetails[msg.sender].startTime + 30 minutes,
  ...
  function completeFlight(address _pilot, uint _flightId) public onlyAdmin nonReentrant returns (uint256) {
    require(block.timestamp >= flightDetails[_pilot].startTime + 30 minutes, 'Flight still in min locked time');
    require(
      keccak256(abi.encodePacked(flightDetails[_pilot].id)) == keccak256(abi.encodePacked(_flightId)),
      'This flight is not active.'
    );

    uint256 _rewards = _calculateRewards(_pilot);

    flightDetails[_pilot] = FlightDetails({id: 0, distance: 0, aircraftId: 0, multiplier: 0, startTime: 0});
