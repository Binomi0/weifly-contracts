...
describe("License D NFT", async function () {
  ...
  it("Any address should be able to claim license D", async () => {
    ...
    const initialBalance = await license.balanceOf(otherAccount.address, 0);
  ...
  it("Any address should be able to claim license D", async () => {
    ...
    const initialBalance = await license.balanceOf(thirdAccount.address, 0);
    expect(initialBalance).to.equal(0);
